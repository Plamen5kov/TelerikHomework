...
var snakeNewPart;
var snakeParts = 1;
var initSnakePosition = randomCell(grid.length - 1); // gets one random position from 1 to length-1
...
function rePosition() {
    var snakePartsArray = stage.get('.snake');
    for (var partIndex = 0; partIndex < snakeParts; partIndex++) {
...
function growSnake(direction) {
    switch (direction) {
        case Down_Arrow:
            ...
            x = snakeHead.getX();
            y = snakeHead.getY() - 10;
            var nHead = createSnakePart(x, y);
            resetPositions(nHead);
...
function initGrid() {
    //*****Initialize Grid
    var grid = [];
    var cols = stageWidth / snakeWidth;
    var rows = stageHeight / snakeHeight;
